const { MasterProfile, Review, Order, Profession } = require('../models');
...
const list = asyncHandler(async (req, res) => {
  let docs = await MasterProfile.find({ is_available: true })
  ...
  if (req.query.professions) {
    ...
    if (ids.length) {
      docs = docs.filter((d) => d.professions.some((p) => ids.includes(String(p._id))));
    }
  }
  ...
  if (req.query.search) {
    ...
    docs = docs.filter((d) => {
      ...
      const proNames = d.professions.map((p) => p.name_uz).join(' ').toLowerCase();
...
const works = asyncHandler(async (req, res) => {
  ...
  res.json(
    orders.map((o) => {
      const ser = masterWorksSerializer(o);
...
const myProfile = asyncHandler(async (req, res) => {
  let profile = await MasterProfile.findOne({ user: req.user._id })
    .populate('user', USER_FIELDS)
    .populate('professions');
  if (!profile) {
...
module.exports = { list, detail, works, myProfile, updateMyProfile, availableOrders, reviewsList, createReview };
