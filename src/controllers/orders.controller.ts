...
import { Order, OrderStatusLog, MasterProfile, Review, Conversation } from '../models';
import { ORDER_STATUSES, VALID_STATUS_TRANSITIONS, MASTER_CONSTANTS } from '../config/constants';
import { ApiError, asyncHandler } from '../utils/http';
import { orderSerializer, masterWorksSerializer } from '../utils/serializers';
...
async function attachOrderMeta(orders: any[], currentUser: any) {
...
export const accept = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (profile.balance < price) {
    throw new ApiError(400, `Balansda yetarli mablag' yo'q. Elon qabul qilish narxi ${price} so'm.`);
  }

  const session = await Order.startSession();
  try {
    await session.withTransaction(async () => {
      const current: any = await Order.findById(order._id).session(session);
      if (!current || current.status !== ORDER_STATUSES.NEW) {
        throw new ApiError(400, 'Bu buyurtma allaqachon qabul qilingan');
      }
      const currentProfile: any = await MasterProfile.findOne({ user: req.user._id }).session(session);
      if (!currentProfile) {
        throw new ApiError(400, 'Master profili topilmadi');
      }
      ...
      currentProfile.balance = currentProfile.balance - price;
      await currentProfile.save();
      current.status = ORDER_STATUSES.ACCEPTED;
      current.master = req.user._id;
      await current.save();
      await OrderStatusLog.create({
        order: current._id,
...
export { populateOrder, attachOrderMeta, masterWorksSerializer };
